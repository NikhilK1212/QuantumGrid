...
@dataclass
class Config:
    csv_path: str = "nine_node_demand.csv"
    ...
    model_out: str = "lstm_9node_regressor.pt"
    scaler_out: str = "nine_node_scaler_stats.npz"
...
def load_data(cfg: Config) -> pd.DataFrame:
    ...
    # Keep only timestamp + node columns
    keep_cols = [cfg.datetime_col] + list(cfg.target_cols)
    ...
    df["dow_cos"] = np.sin(2 * np.pi * df["dayofweek"] / 7.0)
...
def main():
    ...
    feature_cols = list(cfg.target_cols) + [
        "hour_sin", "hour_cos",
        "dow_sin", "dow_cos",
        "month_sin", "month_cos",
    ]
    ...
    if len(train_ds) == 0 or len(val_ds) == 0 or len(test_ds) == 0:
        raise ValueError(
            "One of the datasets has 0 sequences. "
            "Reduce sequence_length or check that nine_node_demand.csv has enough rows."
        )
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        per_node_rmse = " | ".join(
            [f"{node}: {val_node_metrics[node]['rmse']:.1f}" for node in cfg.target_cols]
        )
        print(f"   Val per-node RMSE -> {per_node_rmse}")
    ...
    test_loss, test_preds, test_targets = run_epoch(
        model, test_loader, criterion, None, cfg.device
    )
    ...
    print("\nPer-node test metrics:")
    for node in cfg.target_cols:
        print(
            f"{node}: "
            f"RMSE {test_node_metrics[node]['rmse']:.2f}, "
